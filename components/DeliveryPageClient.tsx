...
import type { DeliveryPageContent } from '@/lib/cms';
...
export function DeliveryPageClient({ content }: { content: DeliveryPageContent }) {
  ...
  async function lookup() {
    ...
    try {
      ...
      if (!response.ok || !data.ok || !data.order) {
        throw new Error(data.error || content.lookupErrorFallback);
      }

      setOrder(data.order);
    } catch (lookupError: any) {
      setOrder(null);
      setError(lookupError?.message || content.lookupErrorFallback);
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="tp-page min-h-screen pb-20 pt-24">
      <div className="container-shell max-w-5xl">
        <section className="rounded-[2rem] border tp-card p-6 md:p-8">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 tp-text-soft">{content.intro}</p>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-[1fr_1fr_0.8fr_auto]">
            ...
            <button type="button" onClick={() => void lookup()} className="btn-primary" disabled={loading}>
              ...
              {loading ? content.searchingButtonLabel : content.searchButtonLabel}
            </button>
          </div>
            ...
            <div className="rounded-[2rem] border tp-card p-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <div className="mt-2 serif-display text-3xl tp-heading">{order.orderNumber}</div>
                </div>
              ...
              <div className="mt-6 grid gap-4">
                <div className="rounded-[1.25rem] bg-[var(--tp-surface)] px-4 py-4">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.16em] tp-text-muted">
                    {content.deliveryWindowLabel}
                  </div>
                  <div className="mt-2 text-sm tp-heading">
                    {order.isCustomOrder ? content.customWindowText : content.standardWindowText}
                  </div>
                </div>
                <div className="rounded-[1.25rem] bg-[var(--tp-surface)] px-4 py-4">
              ...
              <div className="mt-6 rounded-[1.25rem] bg-[var(--tp-surface)] px-4 py-4">
                ...
                <div className="mt-3 space-y-3">
                  {order.notificationLog.length ? (
                    ...
                    <div className="text-sm tp-text-soft">{content.queuedEmptyText}</div>
                  )}
                </div>
              </div>
            </div>
